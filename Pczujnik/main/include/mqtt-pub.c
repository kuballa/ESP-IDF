...
static void mqtt_event_handler(void *handler_args, esp_event_base_t base, int32_t event_id, void *event_data)
{
    ...
    case MQTT_EVENT_CONNECTED:
        ...
        esp_mqtt_client_subscribe(client, MQTT_SUB_ALARM, 0);
        MQTT_CONNECTED = 1;
        break;
    ...
    case MQTT_EVENT_DATA:
        ...
        printf("DATA=%.*s %d %d\r\n", event->data_len, event->data, event->data_len, sizeof event->data);
//        strcpy(alarm_ESP32, event->data);
        prog_alarmu_mV = atof(event->data);
//        printf("%s", alarm_ESP32);
//        task_bluetoothWrite();
        break;
...
void DHT_Publisher_task(void *pvParameter)
{
	...
	while(1) {
		...
		char gas[12];
		char temperatureESP[12];
		char pressureESP[12];
		char alarm_ESP32[12] = "12345";

		sprintf(pressureESP, "%.2f", pressure);
		sprintf(temperatureESP, "%.2f", temperature);
		sprintf(gas, "%.2f", ostatnie_napiecie_na_czujniku_mV);
...
//		printf("Gas %s degC\n\n", gas);
		...
		esp_mqtt_client_publish(client, MQTT_PUB_PRE_DHT, pressureESP, 0, 0, 0);
		esp_mqtt_client_publish(client, MQTT_PUB_TEMP_DHT, temperatureESP, 0, 0, 0);
